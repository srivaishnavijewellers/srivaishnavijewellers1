...
const PRINT_BLOCK_X_MM = 47.5;  // landscape X where the printable content starts
...
const drawText = (ctx, text, x, y, maxW, fontPx) => {
  ...
  ctx.clip();
  // Double-fill sharpens edges on thermal bitmap output.
...
const createPageDataUrl = async (label) => {
  ...
  const txtX = bx + mmToPx(13);                   // text area left edge
  ...
  const valW = mmToPx(TEXT_WIDTH_MM) - mmToPx(12.8); // value column max-width
  const rowH = mmToPx(3);                        // row step (baseline-to-baseline)
  const row0 = by + mmToPx(2.25);               // first row baseline Y
  const lblFz = mmToPx(2.2);                    // label font px
  const valFz = mmToPx(2.7);                    // value font px
